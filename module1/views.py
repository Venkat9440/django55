...
def send_emails(request):
    csv_file_path = "C:\PFSD\DjangoProject\TTM\static\images\email.csv"
    with open(csv_file_path, 'r') as csvfile:
        reader = csv.DictReader(csvfile)
        for row in reader:
            recipient_email = row['email']
            subject = 'respected sir'  # Set your subject here
            message_body = 'Good afternoon sir ,This mail is regarding output verification of insem lab'  # Set your email content here
            send_mail(
                subject,
                message_body,
                'venkatmargani1@gmail.com',  # Update with your sender email
                [recipient_email],
                fail_silently=True,
            )
